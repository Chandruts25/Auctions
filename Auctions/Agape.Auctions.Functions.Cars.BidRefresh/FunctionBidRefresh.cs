...
using Agape.Auctions.Functions.Cars.BidRefresh.Models;
...
{
    public static class FunctionBidRefresh
    {
        ...
        [FunctionName("FunctionBidRefresh")]
        public static async Task RunAsync([CosmosDBTrigger(databaseName: "Tasks", collectionName: "Bidding",
            ConnectionStringSetting = "CosmosDbConnectionstring")] IReadOnlyList<Document> input, ILogger log,
             [SignalR(HubName = "bidRefresh")] IAsyncCollector<SignalRMessage> signalRMessages)
        {
            ...
            {
                log.LogInformation("Exception from  FunctionBidRefresh, Exception Details" + ex.ToString());
            }
        }
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    using (var Response = await client.GetAsync(endpoint))
                    {
                        if (Response.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            var lstUSer = await Response.Content.ReadAsAsync<List<Agape.Auctions.Functions.Cars.BidRefresh.Models.User>>();
